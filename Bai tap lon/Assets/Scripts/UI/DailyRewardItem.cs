...
public class DailyRewardItem : MonoBehaviour {
    ...
    [SerializeField]
    private GameObject imgClaimCheckmark;
    ...
    public void Claim(int factor) {
        ...
        foreach (DailyRewardResourceItem drResourceItem in drResourceItems) {
            if (drResourceItem.resourceItem.detail == ResourceDetail.None) {
                ...
                if (factor == 1)
                {
                    if (drResourceItem.resourceItem.type == ResourceType.BulletNormal)
                    {
                        GameAnalytics.LogGetItem("Bullet", realQuantity, null, 0, 0, "HomeScene", "ui_daily_reward");
                        GameAnalytics.LogUseItem("Bullet", realQuantity, null, "HomeScene", "ui_daily_reward");
                    }
                    else GameAnalytics.LogGetItem(drResourceItem.resourceItem.type.ToString(), realQuantity, null, 0, 0, "HomeScene", "ui_daily_reward");
                }
                else if (factor == 2)
                {
                    if (drResourceItem.resourceItem.type == ResourceType.BulletNormal)
                    {
                        GameAnalytics.LogGetItem("Bullet", realQuantity, "ads", 1, 1, "HomeScene", "ui_daily_reward");
                        GameAnalytics.LogUseItem("Bullet", realQuantity, "ads", "HomeScene", "ui_daily_reward");
                    }
                    else GameAnalytics.LogGetItem(drResourceItem.resourceItem.type.ToString(), realQuantity, "ads", 1, 1, "HomeScene", "claim_x2");
                }
            } else {
