...
public class PlayerAction : MonoBehaviour
{
        ...
        if (collision.gameObject.CompareTag(Constants.TAG.BREAKABLE_BLOCK))
        {
            if (transform.position.y < collision.transform.position.y - 1f)
            {
                if (!isHittingBlock)
                {
                    ...
                    //GameController.instance.GetPoolBreakFx(LevelController.instance.typeMap == TypeMap.Water).Boom(breakBlock.transform.position);                   
                }
            }
        }
    ...
    public void DiePlayer()
    {
        ...
        GameAnalytics.LogPlayerDie(GameData.levelSelected, (int)transform.position.x, (int)transform.position.y, "flint");  
    }
    ...
    public void HurtPlayer(int damage)
    {
        if (!isGetHurt && !PlayerMovement.instance.isProtected)
        {
            ...
            GameData.heartLose++;
            UIGameController.instance.UpdateHeart(totalHealth);
            if (totalHealth <= 0)
            {
                DiePlayer();
            }
    ...
    public void MoveInDoor()
    {
        ...
        GameAnalytics.LogFirebaseUserProperty("total_heart", GameData.realHealth);
        GameAnalytics.LogFirebaseUserProperty("total_acorn", GameData.Bullet);
        GameAnalytics.LogUIAppear("popup_complete", "GamePlay");
        GameAnalytics.LogLevelEnd(GameData.levelSelected, true, GameData.levelPlayedTime, GameData.levelPlayedTime, GameData.SelectedCharacter.ToString(), "complete_level", GameData.bulletUse, GameData.heartLose);       
    }
    ...
    public void FinishLevel()
    {
        ...
        playerMovement.anim.SetTrigger("Happy");
        if (playerMovement.posHand.childCount != 0)
        {
            playerMovement.posHand.GetChild(0).gameObject.SetActive(false);
        }
        StartCoroutine(Helper.StartAction(() => transform.DOMoveX(Door.instance.PosWin.transform.position.x, 2f).OnComplete(() =>
        {
            playerMovement.enabled = true;
            playerMovement.groundCheck.enabled = true;                        
        }), 1.1f));
        StartCoroutine(Helper.StartAction(() => playerMovement.anim.SetFloat("Speed", 1), 1.1f));
        StartCoroutine(Helper.StartAction(() => Door.instance.CloseDoor(), 4f));
    }
}
